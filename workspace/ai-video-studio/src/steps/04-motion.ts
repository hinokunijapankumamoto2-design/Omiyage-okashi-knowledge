    ...
    if (klingRequests.length > 0) {
      requestAgentWork(project, {
        ...
        instruction: [
          ...
          '',
          `第一候補: ${TOOL_IDS.klingGenerateVideo}（Kling MCP）`,
          `Kling MCP が未接続なら代替: ${TOOL_IDS.klingFallback} に model=kling3_0_turbo を指定`,
          '',
          '各リクエストの startImagePath は projects/<id>/ からの相対パスです。',
          'MCP に渡す前に media_upload 系でアップロードし、返った media_id を使ってください。',
          '',
